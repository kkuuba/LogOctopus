...
def test_normal_log_collection_scenario():
    ...
    for log_name in example_device.log_snapshots[-1].collected_data:
        assert example_device.log_snapshots[-1].logs_collection_duration[log_name] >= 0
    assert example_device.log_snapshots[-1].size_in_bytes > 0
    data_file_names = example_device.log_snapshots[-1].create_parquet_data_file("example_device")
